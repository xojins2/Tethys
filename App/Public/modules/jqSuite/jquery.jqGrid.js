    ...
    $.jgrid.extend({
        ...
        excelExport : function(o) {
            ...
            return this.each(function(){
                ...
                var url;
                if(o.exptype == "remote") {
                    var pdata = $.extend({},this.p.postData);    //object with post vars
                    pdata[o.oper] = o.tag;                       //replace the oper with the export type
                    //var params = jQuery.param(pdata);            //create a string that will be appended to the query string
                    //if(o.url.indexOf("?") != -1) { url = o.url+"&"+params; }
                    //else { url = o.url+"?"+params; }             //append the string to the url
                    //window.location = url;                       //call the url
                    var post_params = {};
                    for (var pkey in pdata){
                        //create a form and make all the values as hidden
                        //then submit the form to the original url
                        post_params[pkey] = pdata[pkey];
                    }
                    //post form
                    //$.post(o.url,post_params);
                    $.download(o.url,post_params,'post');
                }
            });
        }
    });
